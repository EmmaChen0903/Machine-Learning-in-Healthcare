...
def count_n_days(df_master, n_days):    
    ...
    df_icu = df_master[['index', 'subject_id', 'intime_ed', 'intime_icu']] # extract columns needed for computation of past ICU count
    ...
    df_icu = df_icu[['index', 'n_icu_'+str(n_days)+'d']] # only keep columns for merging
    df_master = df_master.merge(df_icu, on='index', how='left') # merge updated past ICU count to df_master
