...
def read_admissions_table(admissions_table_path):
    ...
    df_admissions =  df_admissions[['subject_id', 'hadm_id', 'admittime', 'dischtime', 'deathtime','race', 'edregtime','edouttime', 'insurance']]
...
def add_outcome_icu_transfer(df_master, df_icustays, timerange):
    ...
    df_icustays_sorted = df_icustays[['subject_id', 'hadm_id', 'intime','outtime']].sort_values('intime')
    # print(df_icustays_sorted)
    df_icustays_keep_first = df_icustays_sorted.groupby('hadm_id').first().reset_index()
    df_master_icu = pd.merge(df_master, df_icustays_keep_first, on = ['subject_id', 'hadm_id'], how='left', suffixes=('_ed','_icu'))
    time_diff = (df_master_icu['intime_icu']- df_master_icu['outtime_ed'])
...
def fill_na_race(df_master): # requires df_master to be sorted 
    N = len(df_master)
    race_list= [float("NaN") for _ in range(N)]
    race_dict = {} # dict to store subejct race

    def get_filled_race(row):
        i = row.name
        if i % 10000 == 0:
        ...
        if curr_subject not in race_dict.keys(): ## if subject race not stored yet, look ahead and behind 
            subject_race_list = []
            ...
            subject_race_list.append(df_master['race'][i]) ## add current race to list

            while prev_subject == curr_subject:
                subject_race_list.append(df_master['race'][prev_subject_idx])
                prev_subject_idx -= 1
                prev_subject= df_master['subject_id'][prev_subject_idx] if prev_subject_idx >= 0 else None

            while next_subject == curr_subject:
                subject_race_list.append(df_master['race'][next_subject_idx])
                next_subject_idx += 1
                next_subject= df_master['subject_id'][next_subject_idx] if next_subject_idx <= (N-1) else None
        
            race_counter_list = collections.Counter(subject_race_list).most_common() #sorts counter and outputs list
            
            if len(race_counter_list) == 0: ## no previous or next entries 
                subject_race = curr_race
            elif len(race_counter_list) == 1: ## exactly one other race
                subject_race = race_counter_list.pop(0)[0] ## extract race from count tuple
            else:
                race_counter_list = [x for x in race_counter_list if pd.notna(x[0])] # remove any NA
                subject_race = race_counter_list.pop(0)[0]
            
            race_dict[curr_subject] = subject_race ## store in dict
    
        if pd.isna(curr_race): ## if curr_race is na, fill with subject_race from dict
            race_list[i]= race_dict[curr_subject]
        else:
            race_list[i]= curr_race
            
    df_master.apply(get_filled_race, axis=1)
    print('Process: %d/%d' % (N, N), end='\r')
    df_master.loc[:,'race'] = race_list
    return df_master


def generate_past_ed_visits(df_master, timerange):
    #Requires sorted data by subject ID and intime
    ...
    def get_num_past_admissions(df):
        ...
        if subject_id in grouped_adm.groups.keys():
            ...
            for i in df.index:
                if i % 10000 == 0:
                    print('Process: %d/%d' % (i, N), end='\r')
                while start < df_adm.index[-1] and df.loc[i, 'intime_ed'] - df_adm.loc[start, 'admittime'] > timerange_delta:
                    start += 1
                end = start
                while end <= df_adm.index[-1] and \
                        (timerange_delta >= (df.loc[i, 'intime_ed'] - df_adm.loc[end, 'admittime']) > timedelta(days=0)):
                    end += 1
                n_adm[i] = end - start
...
def generate_past_icu_visits(df_master, df_icustays, timerange):
    # df_icustays.rename({'intime':'intime_icu','outtime':'outtime_icu'},axis=1,inplace=True)
    df_icustays_sorted = df_icustays[df_icustays['subject_id'].isin(df_master['subject_id'].unique().tolist())][['subject_id', 'intime_icu']].copy()
    df_icustays_sorted.sort_values(['subject_id', 'intime_icu'], inplace=True)
    ...
    def get_num_past_icu_visits(df):
        ...
        if subject_id in grouped_icu.groups.keys():
            ...
            for i in df.index:
                if i % 10000 == 0:
                    print('Process: %d/%d' % (i, N), end='\r')
                while start < df_icu.index[-1] and df.loc[i, 'intime_icu'] - df_icu.loc[start, 'intime_icu'] > timerange_delta:
                    start += 1
                end = start
                while end <= df_icu.index[-1] and \
                        (timerange_delta >= (df.loc[i, 'intime_icu'] - df_icu.loc[end, 'intime_icu']) > timedelta(days=0)):
                    end += 1
                n_icu[i] = end - start
...
def generate_future_ed_visits(df_master, next_ed_visit_timerange):
    ...
    def get_future_ed_visits(row):
        ...
        if curr_subject == next_subject:
            curr_outtime = row['outtime_ed']
            next_intime = df_master['intime_ed'][i+1]
